...
func makeWorker(
	...
	w := New(cfg, steps, builds, projects, workers, tokens, nil, git, ws, runner, &mockStatusPoster{}, nil, nil, NoopLogUploader{}, nil)
	w.id = "test-worker"
	return w
}
...
func TestSkipsWhenAtCapacity(t *testing.T) {
	...
	w := New(cfg, steps, builds, projects, workers, tokens, nil, gitSvc, ws, runner, &mockStatusPoster{}, nil, nil, NoopLogUploader{}, nil)
	w.id = "test-worker"
